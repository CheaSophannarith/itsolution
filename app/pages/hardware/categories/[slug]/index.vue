    ...
    <div class="min-h-screen">
        ...
        <div class="flex mx-auto max-w-7xl px-6 py-12 gap-16">
            ...
            <aside class="w-64 shrink-0">
                ...
                <div class="mb-6">
                    <h2 class="text-xl font-bold text-gray-800 mb-4">Hardware</h2>
                    <div class="space-y-1">
                        <div v-for="cat in categories" :key="cat.id" class="border-b border-gray-100 last:border-b-0">
                            <div
                                class="w-full flex items-center justify-between py-2 px-2 text-left text-sm font-medium hover:bg-gray-50 rounded transition-colors"
                                :class="cat.id === category?.id ? 'text-brand bg-brand/5' : 'text-gray-700'"
                            >
                                <NuxtLink
                                    :to="`/hardware/categories/${cat.slug}`"
                                    class="flex-1 hover:text-brand"
                                >
                                    {{ cat.name }}
                                </NuxtLink>
                                <button @click="toggleCategory(cat.id)" class="p-1 hover:bg-gray-200 rounded">
                                    <ChevronDown
                                        class="w-4 h-4 transition-transform duration-200"
                                        :class="isCategoryExpanded(cat.id) ? 'rotate-180' : ''"
                                    />
                                </button>
                            </div>
                            <div
                                v-show="isCategoryExpanded(cat.id)"
                                class="pl-4 pb-2 space-y-1"
                            >
                                <NuxtLink
                                    v-for="sub in cat.subcategories"
                                    :key="sub.id"
                                    :to="`/hardware/categories/${cat.slug}/${sub.slug}`"
                                    class="block py-1.5 px-2 text-sm text-gray-600 hover:text-brand hover:bg-gray-50 rounded transition-colors"
                                >
                                    {{ sub.name }}
                                </NuxtLink>
                            </div>
...
<script setup lang="ts">
    ...
    import { ChevronRight, ChevronLeft, ChevronDown, Star, Package, Search } from 'lucide-vue-next';
