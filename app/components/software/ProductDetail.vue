            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                <Breadcrumb>
                    <BreadcrumbList>
                        ...
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <BreadcrumbLink :href="`/software/categories/${category?.slug}`">
                                {{ category?.name }}
                            </BreadcrumbLink>
                        </BreadcrumbItem>
                        <BreadcrumbSeparator v-if="subcategory" />
                        <BreadcrumbItem v-if="subcategory">
                            <BreadcrumbLink :href="`/software/categories/${category?.slug}/${subcategory?.slug}`">
                                {{ subcategory?.name }}
                            </BreadcrumbLink>
                        </BreadcrumbItem>
                        <BreadcrumbSeparator />
            ...
            <div class="flex flex-col lg:flex-row gap-8">
                    ...
                    <div class="lg:w-1/2 space-y-5">
                        ...
                        <div class="border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <div class="p-4">
                                <div class="space-y-2">
                                    <div v-if="brand" class="flex items-start text-sm py-2 border-b border-gray-100">
                                        <span class="font-medium text-gray-600 w-24 shrink-0">Brand</span>
                                        <span class="text-gray-800">{{ brand.name }}</span>
                                    </div>
                                    <div class="flex items-start text-sm py-2 border-b border-gray-100">
                                        <span class="font-medium text-gray-600 w-24 shrink-0">Category</span>
                                        <span class="text-gray-800">{{ category?.name }}</span>
                                    </div>
                                    <div class="flex items-start text-sm py-2 border-b border-gray-100">
                                        <span class="font-medium text-gray-600 w-24 shrink-0">Rating</span>
                                        <span class="text-gray-800 flex items-center gap-1">
                                            {{ product.rating }}
                                            <Star class="w-4 h-4 fill-yellow-400 text-yellow-400" />
                                        </span>
                                    </div>
                                    <div class="flex items-start text-sm py-2">
                                        <span class="font-medium text-gray-600 w-24 shrink-0">Description</span>
            ...
            <div class="mt-12 bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                ...
                <div class="bg-gradient-to-r from-pink-500 to-pink-600 px-6 py-4">
                    <div class="flex items-center justify-between">
                        ...
                        <NuxtLink v-if="category" :to="`/software/categories/${category.slug}`"
                ...
                <div class="p-6 bg-gray-50">
                    <div v-if="similarProducts.length > 0"
                        ...
                        <div v-for="similarProduct in similarProducts" :key="similarProduct.id"
                            ...
                            <SoftwareProductCard :product="similarProduct" />
                        </div>
                    </div>
...
<script setup lang="ts">
    ...
    import { CheckCircle, ChevronRight, Info, Minus, Package, Plus, ShoppingCart, Star } from 'lucide-vue-next';
    import {
        Breadcrumb,
        BreadcrumbItem,
        BreadcrumbLink,
    ...
    import brandsData from '~/assets/data/Software/brand.json';
