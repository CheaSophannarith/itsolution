<template>
    <div class="bg-white rounded-2xl border border-gray-200 shadow-md hover:shadow-xl transition-all duration-300 min-h-75 flex flex-col justify-between">
        <!-- Card Link -->
        <NuxtLink :to="`/categories/${category.slug}`" class="block px-6 pt-6 pb-2 group">
            <!-- Category Icon -->
            <div class="flex justify-center mb-4">
                <div class="w-28 h-28 bg-linear-to-tr from-blue-100 to-blue-200 rounded-xl overflow-hidden shadow group-hover:scale-105 group-hover:shadow-lg transition-transform duration-300 flex items-center justify-center">
        ...
        <div v-if="category.children && category.children.length > 0" class="px-4 pb-5">
            <button @click.stop="toggleSubcategories"
                ...
                <span class="flex items-center justify-center text-lg font-bold">{{ isExpanded ? '-' : '+' }}</span>
